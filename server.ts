...
await ensureTables();await seedOwner();const existing=await db.select().from(S.settings).where(eq(S.settings.key,"site"));if(!existing.length)await db.insert(S.settings).values({key:"site",value:defaults,updatedAt:now()});for(const [sid,title,text] of serviceDefaults){const ex=await db.select().from(S.services).where(eq(S.services.id,sid));if(!ex.length)await db.insert(S.services).values({id:sid,title,text,image:null,createdAt:now(),updatedAt:now()})}await cleanupSessions();console.log("Charan Mobiles running on http://localhost:"+(process.env.PORT||3000));export default {port:Number(process.env.PORT||3000),fetch:app.fetch};
